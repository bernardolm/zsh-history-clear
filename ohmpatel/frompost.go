...
func main() {
	...
	if lastLogCreationTime.After(queryStartTime) && lastLogCreationTime.Before(queryFinishTime) {
		_Process(file, queryStartTime, queryFinishTime)
	}

	fmt.Println("\nTime taken - ", time.Since(s))
}

func _Process(f *os.File, start time.Time, end time.Time) error {
	...
	for {
		...
		wg.Add(1)
		go func() {
			_ProcessChunk(buf, &linesPool, &stringPool, start, end)
			wg.Done()
		}()
...
func _ProcessChunk(chunk []byte, linesPool *sync.Pool, stringPool *sync.Pool, start time.Time, end time.Time) {
